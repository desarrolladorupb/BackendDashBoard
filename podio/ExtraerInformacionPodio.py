...
class ExtraerInformacionPodio():
    ...
    def getFieldValue(self, field, _client, no_html=False, external_id=True, depth=1, lstFields=[]):
        """
        ...
        if(len(lstFields) < 1):
            realizar = True
        elif field["external_id"] in lstFields:
            realizar = True
        else:
            realizar = False
    ...
    def makeDict(self, item, nested=False, no_html=False, lstFields = []):
        """
        Creates a dictionary with the external_id of the item's fields ad keys, and their values as the dictionary values.
        """
        dictionary = dict(
            [(field["external_id"], self.getFieldValue(field, nested, no_html, lstFields=lstFields)) for field in item["fields"]])
